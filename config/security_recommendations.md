...
- **Risk**: `http:` and `https:` whitelist _every_ domain on the internet, nullifying protections against malicious external scripts. `'unsafe-inline'` and `'unsafe-eval'` leave the app vulnerable to XSS.
...
- **Recommendation**: Ensure _every_ sensitive Server Action (e.g., `deleteDeck`, `updateUser`) is wrapped with these checks.
