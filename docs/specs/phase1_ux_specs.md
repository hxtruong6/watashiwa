...
- **Hero Content**: The _Kanji_ is the protagonist. Everything else is supporting cast.
...
2. User taps "Start".
3. **Transition**: The Dashboard elements _fade out_, and the first card _glides in_ from the bottom-right (stack effect).
4. **First Card**: It is NOT a test. It is a **Priming Card** (e.g., "Reviewing: Travel Context").
   - _Visual_: A soft, translucent overlay or a "Cover Card".
   - _Action_: User taps "Ready". Card slides away to reveal the first challenge.
...
4. **Reveal**: User taps card.
   - _Animation_: `rotateY(180deg)` (3D Flip).
   - _Back Face_: The **Etymology Breakdown** animates in sequence:
     - `先` (Before) + `生` (Life) = "One born before" -> Teacher.
     - Standard Meaning: "Teacher, Master".
5. **Rating**: User selects "Good".
   - _Feedback_: Card glows soft Green (`colorSuccess`). Slides off-screen to the _Right_.
...
6. **Intervention Card (Modal)**:
   - _Visual_: Split screen. Left: 🥢 (High-Low). Right: 🌉 (Low-High).
   - _Action_: "Listen & Tap".
   - _Goal_: User fixes the auditory mapping _immediately_.
7. **Recovery**: User completes intervention. Original card returns to the _end_ of the short-term queue.
...
2. **Swipe Left**: "Again" (Rating 1). _Card tilts Red._
3. **Swipe Right**: "Good" (Rating 3). _Card tilts Green._
4. **Swipe Up**: "Easy" (Rating 4). _Card tilts Blue._
...
_Effect_: As Active slides away, Next _springs up_ to become Active.
