...
> **Objective:** Build the _permanent_ frontend architecture and data structures now, but feed them with _static/simple_ seed data for this milestone.
...
export interface Vocabulary {
	...
	han_viet: {
		// Maps to `hanViet` string or parsed if needed
		chars: string;
		breakdown: string[];
	};
...
**Concept:** This is a _Hybrid Type_. It combines:
...
1. [x] **Create `types.ts`**: Define the strict `Vocabulary` and `SmartCard` union types.
   - _Constraint:_ Ensure `mnemonic` matches Prisma.
