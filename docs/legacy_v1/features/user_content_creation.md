...
- **Visual Cards**: Instead of a spreadsheet view, show items as _Cards_.
  - _List View_: Compact, for overview.
  - _Grid View_: Visual cards showing the front/back.
...
- **Kanji Breakdown Visualizer**:
  - When a user types a compound (e.g., "学生"), automatically chip the breakdown: [学 (Study)] + [生 (Life)]. This visualizes _why_ the word makes sense.
...
- **Discovery**: The current logic _already_ queries for new Vocab/Kanji where `deck.authorId === user.id`.
