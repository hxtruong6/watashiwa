...
The user asked: *"Does Firebase Notifications support our features?"*
...
* **Why?**: It tells the Push Service (Google/Apple) that *we* are the sender. No API keys stored in the app.
...
* **Solution**:
  ...
  * *Do not skip this*, or we will be rate-limited by Google/Apple.
...
* `NotificationListener` listens and decides *if* and *how* to notify (Push vs Email vs In-App).
