...
// Current StandardFace.tsx (lines 349-393)
{
	hasExample && (
		<div style={{ marginBottom: hasMnemonic ? '24px' : '0' }}>
			// ❌ Always visible // ❌ No collapse option // ❌ Japanese + Vietnamese translation = 4+
			lines of text
		</div>
	);
}
...
```typescript
const shouldShowSidebar = showAnswer && (relatedWords.length > 0 || hasConfusions || hasEtymology);
```
